...
class Helpers {
	...
	public static function download_import_files( $import_file_info ) {
		...
		$import_file_info = apply_filters('ocdi/pre_download_import_files', $import_file_info);
		...
		if ( empty( $import_file_info['import_file_url'] ) ) {
			...
			$content_filename = apply_filters( 'ocdi/downloaded_content_file_prefix', 'demo-content-import-file_' ) . self::$demo_import_start_time . apply_filters( 'ocdi/downloaded_content_file_suffix_and_file_extension', '.xml' );

			// Download the content import file.
			if(is_array($import_file_info['import_file_url'])) {
				$downloaded_files['content'] = array();
				foreach($import_file_info['import_file_url'] as $key => $xml_file) {
					if(!$xml_file) continue;

					$downloaded_file = $downloaded_files['content'][] = $downloader->download_file( $xml_file, str_replace('.xml', '-'.$key.'.xml', $content_filename) );
				...
				// Return from this function if there was an error.z
		...
		if ( ! empty( $import_file_info['import_widget_file_url'] ) ) {
			...
			$widget_filename = apply_filters( 'ocdi/downloaded_widgets_file_prefix', 'demo-widgets-import-file_' ) . self::$demo_import_start_time . apply_filters( 'ocdi/downloaded_widgets_file_suffix_and_file_extension', '.json' );
		...
		if ( ! empty( $import_file_info['import_customizer_file_url'] ) ) {
			...
			$customizer_filename = apply_filters( 'ocdi/downloaded_customizer_file_prefix', 'demo-customizer-import-file_' ) . self::$demo_import_start_time . apply_filters( 'ocdi/downloaded_customizer_file_suffix_and_file_extension', '.dat' );
		...
		if ( ! empty( $import_file_info['import_redux'] ) && is_array( $import_file_info['import_redux'] ) ) {
			...
			foreach ( $import_file_info['import_redux'] as $index => $redux_item ) {
				$redux_filename = apply_filters( 'ocdi/downloaded_redux_file_prefix', 'demo-redux-import-file_' ) . $index . '-' . self::$demo_import_start_time . apply_filters( 'ocdi/downloaded_redux_file_suffix_and_file_extension', '.json' );
	 ...
	 * With 3.0 we changed the filter prefix from 'ocdi/' to just 'ocdi/',
	 ...
	 * With 3.0 we changed the action prefix from 'ocdi/' to just 'ocdi/',
	 * but we needed to make sure backwards compatibility is in place.

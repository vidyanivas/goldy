...
class OneClickDemoImport {
  ...
  private $plugin_page_setup = array();
	...
	public function create_plugin_page() {
		$this->plugin_page_setup = apply_filters( 'ocdi/plugin_page_setup', array(
			'parent_slug' => 'novo_dashboard',
			//'parent_slug' => 'themes.php',
			'page_title'  => esc_html__( 'One Click Demo Import' , 'ocdi' ),
			'menu_title'  => esc_html__( 'Import Demo Content' , 'ocdi' ),
			'capability'  => 'import',
		...
		if(get_option('enable_full_version')) {
			$this->plugin_page = add_submenu_page(
				...
				apply_filters( 'ocdi/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) )
			);
		}

		register_importer( $this->plugin_page_setup['menu_slug'], $this->plugin_page_setup['page_title'], $this->plugin_page_setup['menu_title'], apply_filters( 'ocdi/plugin_page_display_callback_function', array( $this, 'display_plugin_page' ) ) );
	}
		...
		if ( $this->plugin_page === $hook || ( 'admin.php' === $hook && $this->plugin_page_setup['menu_slug'] === esc_attr( $_GET['import'] ) ) ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'import_popup'     => apply_filters( 'ocdi/enable_grid_layout_import_popup_confirmation', true ),
					'theme_screenshot' => $theme->get_screenshot(),
					'import_parts' => array(
						'theme_options' => get_option('yprm_import_theme_options', true),
						'posts' => get_option('yprm_import_posts', true),
						'projects' => get_option('yprm_import_projects', true),
						'products' => get_option('yprm_import_products', true),
						'widgets' => get_option('yprm_import_widgets', true),
						'contact_forms' => get_option('yprm_import_contact_forms', true),
					),
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'ocdi' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'ocdi' ),
						'dialog_no'             => esc_html__( 'Cancel', 'ocdi' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'ocdi' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'ocdi' ),
					),
					'dialog_options' => apply_filters( 'ocdi/confirmation_dialog_options', array() )
				)
			);
	...
	public function setFist() {
		add_option('yprm_import_theme_options', false);
		add_option('yprm_import_posts', false);
		add_option('yprm_import_projects', false);
		add_option('yprm_import_products', false);
		add_option('yprm_import_widgets', false);
		add_option('yprm_import_contact_forms', false);
	}
	...
	public function import_demo_data_ajax_callback() {
		...
		ini_set( 'memory_limit', apply_filters( 'ocdi/import_memory_limit', '350M' ) );
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'ocdi' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: ocdi/import_files.
				...
				if($_POST['content'] === 'false') {
					$this->import_files[ $this->selected_index ]['import_file_url']['content'] = '';
					$this->import_files[ $this->selected_index ]['import_file_url']['elementor_content'] = '';
				} else if($_POST['is_elementor'] == 'true' && $_POST['content'] !== "false") {
					$this->import_files[ $this->selected_index ]['import_file_url']['content'] = $this->import_files[ $this->selected_index ]['import_file_url']['elementor_content'];
				}

				$this->import_files[ $this->selected_index ]['import_file_url']['elementor_content'] = '';

				$checkFields = array( 'media_files', 'header', 'posts', 'projects', 'products', 'contact_forms' );
				...
				foreach($checkFields as $field) {
					...
					if($_POST[$field] === "false") {
						$this->import_files[ $this->selected_index ]['import_file_url'][$field] = '';
					}
				}

        if($_POST['footer'] === "false") {
          $this->import_files[ $this->selected_index ]['import_file_url']['footer-elementor'] = '';
          $this->import_files[ $this->selected_index ]['import_file_url']['footer'] = '';
        } else {
          if($_POST['is_elementor'] == 'true') {
            $this->import_files[ $this->selected_index ]['import_file_url']['footer'] = '';
          } else {
            $this->import_files[ $this->selected_index ]['import_file_url']['footer-elementor'] = '';
          }
				...
				if(
          $this->import_files[ $this->selected_index ]['import_file_name'] == 'Full White' ||
          $this->import_files[ $this->selected_index ]['import_file_name'] == 'Full Dark'
        ) {
					$_POST['theme_options'] = $_POST['widgets'] = 'true';
				}

				if(isset($_POST['theme_options']) && $_POST['theme_options'] == "false") {
					$this->import_files[ $this->selected_index ]['import_redux'] = '';
				}
				if(isset($_POST['widgets']) && $_POST['widgets'] == "false") {
					$this->import_files[ $this->selected_index ]['import_widget_file_url'] = '';
				}
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'ocdi' )
					);
				}
				...
				$log_added = Helpers::append_to_file(
					sprintf(
						__( 'The import files for: %s were successfully downloaded!', 'ocdi' ),
				...
				wp_send_json( esc_html__( 'No import files specified!', 'ocdi' ) );
			}
		}
		...
		if ( ! $this->before_import_executed ) {
			 ...
			 */
			do_action( 'ocdi/before_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
		...
		if ( ! empty( $this->selected_import_files['content'] ) ) {
			if(is_array($this->selected_import_files['content'])) {
				foreach($this->selected_import_files['content'] as $file) {
					$this->setup_plugin_with_filter_data();
					
					$this->append_to_frontend_error_messages( $this->importer->import_content( $file ) );
				}
			} else {
		 ...
		 */
		do_action( 'ocdi/after_content_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		...
		if ( false !== has_action( 'ocdi/after_all_import_execution' ) ) {
	...
	public function import_customizer_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'ocdi/customizer_import_execution', $this->selected_import_files );
		}
	...
	public function after_all_import_data_ajax_callback() {
		...
		if ( $this->use_existing_importer_data() ) {
			 ...
			 */
			do_action( 'ocdi/after_all_import_execution', $this->selected_import_files, $this->import_files, $this->selected_index );
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'ocdi' ),
	...
	public function load_textdomain() {
		load_plugin_textdomain( 'ocdi', false, plugin_basename( dirname( dirname( __FILE__ ) ) ) . '/languages' );
	}
	...
	public function setup_plugin_with_filter_data() {
		...
		$this->import_files = Helpers::validate_import_file_info( apply_filters( 'ocdi/import_files', array() ) );
		...
		$importer_options = apply_filters( 'ocdi/importer_options', array(
			'fetch_attachments' => true,
		) );

		// Logger options for the logger used in the importer.
		$logger_options = apply_filters( 'ocdi/logger_options', array(

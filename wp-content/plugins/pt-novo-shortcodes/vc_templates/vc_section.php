...
$css_classes[] = $block_id = 'vc_section-'.$uniqid;

if($content_color == 'black') {
  $css_classes[] = 'dark-color';
} else if($content_color == 'white') {
  $css_classes[] = 'light-color';
}
...
if($section_name) {
  $wrapper_attributes[] = 'data-section-nav="'.yprm_camel_case($section_name).'" data-section-title="'.esc_attr($section_name).'"';
}

$output .= '<section ' . implode( ' ', $wrapper_attributes ) . '>';
  if ($bg_overlay = yprm_build_bg_overlay($atts, $block_id)) { 
    $output .= $bg_overlay;
  }

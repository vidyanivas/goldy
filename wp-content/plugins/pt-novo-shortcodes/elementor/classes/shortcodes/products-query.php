...
class Pt_Product_Query_Class extends Pt_Query_Interface {
	...
	public function parse_query_args() {
		$query_args = [
			...
			'order'               => strtoupper( $this->get_att( 'order' ) ),
		];
		...
		$query_args = apply_filters( 'novo_woocommerce_widget_products_query', $query_args );
	...
	protected function set_featured_query_args( &$query_args ) {
		if ( 'featured' === $this->get_att( 'sort_type' ) ) {
	...
	protected function set_sale_products_query_args( &$query_args ) {
		if ( 'sale' === $this->get_att( 'sort_type' ) ) {
	...
	protected function set_ids_query_args( &$query_args ) {
		...
		switch ( $this->get_att( 'sort_type' ) ) {
			case 'by_id':
				$post__in = $this->get_att( 'posts_ids' );
				break;
			case 'sale':
	...
	private function set_terms_query_args( &$query_args ) {
		...
		$query_type = $this->get_att( 'sort_type' );

		if ( in_array( $query_type, [ 'by_id' ], true ) ) {
			return;
		}

		if ( empty( $this->get_att( 'include' ) ) || empty( $this->get_att( 'include_term_ids' ) ) || ! in_array( 'terms', $this->get_att( 'include' ), true ) ) {
			return;
		}

		$terms = [];
		foreach ( $this->get_att( 'include_term_ids' ) as $id ) {
	...
	protected function set_include_query_args( &$query_args ) {
		...
		if ( empty( $this->get_att( 'include' ) ) || empty( $this->get_att( 'include_ids' ) ) ) {
			return;
		}

		$post__in = [];

		$post__in = array_merge( $post__in, $this->get_att( 'include_ids' ) );

		$query_args['post__in'] = empty( $query_args['post__in'] ) ? $post__in : array_merge( $query_args['post__in'], $post__in );

		if ( 'sale' === $this->get_att( 'sort_type' ) ) {
			$query_args['post__in'] = array_diff( $query_args['post__not_in'], $query_args['post__in'] );
		}
	}

	protected function set_exclude_query_args( &$query_args ) {
		if ( empty( $this->get_att( 'exclude' ) ) ) {
			return;
		}
		$post__not_in = [];
		if ( in_array( 'current_post', $this->get_att( 'exclude' ) ) ) {
			if ( is_singular() ) {
				$post__not_in[] = get_queried_object_id();
			}
		}

		if ( in_array( 'manual_selection', $this->get_att( 'exclude' ) ) && ! empty( $this->get_att( 'exclude_ids' ) ) ) {
			$post__not_in = array_merge( $post__not_in, $this->get_att( 'exclude_ids' ) );
		}
		...
		if ( in_array( 'terms', $this->get_att( 'exclude' ) ) && ! empty( $this->get_att( 'exclude_term_ids' ) ) ) {
			$terms = [];
			foreach ( $this->get_att( 'exclude_term_ids' ) as $to_exclude ) {
	...
	protected function set_best_sellings_products_query_args( &$query_args ) {
		if ( 'best_selling' === $this->get_att( 'sort_type' ) ) {

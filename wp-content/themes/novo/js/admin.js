jQuery(document).ready(function() {
  ...
  jQuery(document).on('change', '.yprm-input.required, .wis-input.required', function() {
    var $el = jQuery(this);
  ...
  jQuery('.yprm-validation-form button.yprm-button, .wis-validation-form button.wis-button').on('click', function(e) {
    e.preventDefault();
    ...
    $this.find('.yprm-input, .wis-input').each(function() {
      if(!jQuery(this).val()) {
    ...
    jQuery.ajax({
      ...
      success: function (response) {
        ...
        if(result == null || result == 'null' || typeof result.result == 'undefined' || result.result == 'access_denied' || result.result == 'remove_success') {
          ...
          $this.find('.yprm-input-row, .wis-input-row').removeClass('is-ok');
          $this.find('.deregister').fadeOut();
          $this.find('.yprm-message.is-ok, .wis-message.is-ok').removeClass('is-ok').text('You can register one license per one website.');
          ...
          $this.find('.yprm-log-massage, .wis-log-massage').removeClass('is-ok').addClass('is-error').html(error_message).slideDown().delay(delay).slideUp();
          $this.find('.yprm-button.register, .wis-button.register').show();
          $this.find('.yprm-button.deregister, .wis-button.deregister').hide();
        } else if(result.result == 'access_success') {
          $this.find('.yprm-input-row, .wis-input-row').addClass('is-ok');
          $this.find('.yprm-log-massage, .wis-log-massage').removeClass('is-error').addClass('is-ok').text('Congratulations, Your theme is activated.').slideDown();
          $this.find('.yprm-button.register, .wis-button.register').hide();
          $this.find('.yprm-button.deregister, .wis-button.deregister').show();
        }
